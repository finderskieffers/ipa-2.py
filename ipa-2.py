...
def vigenere_cipher(message, key):
    ...
    for char in message:
        if char == " ":
            ...
            keychar = key[keyind%len(key)]
            keycharind = alphabet.index(keychar)
            ind2 = (ind1+keycharind)%len(alphabet)
            encrypted.append(alphabet[ind2])
        keyind += 1
...
def scytale_cipher(message, shift):
    ...
    for i in range(columns):
        for j in range(shift):
            encrypted.append(message[j * columns + i])
...
def scytale_decipher(message, shift):
    ...
    for i in range(shift):
        for j in range(columns):
            decrypted.append(message[j * shift + i])
